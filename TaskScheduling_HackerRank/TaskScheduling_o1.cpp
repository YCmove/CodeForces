...
struct N { // data to store in tree
    // operation load.log update.,
    // subtree statistic(calculate sub tree status in every update)
    LL each, maxsub;
    N() {
        each = 0;
        maxsub = -INF;
...
LL query2(int ni, int s, int e, int ps, int pe) {
    ...
    if (pe < s || ps > e)
        ...
        int mid = (s + e) / 2;
        return max(query2(2 * ni + 1, s, mid, ps, min(pe, mid)),
                   query2(2 * ni + 2, mid + 1, e, max(ps, mid + 1), pe)) +
               st[ni].each;
    }
}
